...
### 支持的图像分辨率
- 预设分辨率：320×256, 640×512 (默认), 800×600, 1024×768, 1280×1024, 1920×1080
- **自定义分辨率**：支持用户输入任意宽度和高度（1-10000像素）
...
- **文件偏移**: 设置从文件的第N个字节开始读取数据（用于跳过文件头）

**实时刷新**: 修改任何参数后，图像和十六进制视图会自动刷新显示
...
- 点击 **暂停** 按钮停止播放
- 当前帧号和总帧数显示在控制面板
- 切换帧时，图像、直方图和十六进制视图同步更新

#### 6. 十六进制查看
- 切换到 **十六进制视图** 标签页
- 查看当前图像数据的原始二进制内容
- 每行显示：地址偏移量 | 十六进制数据 | ASCII字符
- **智能显示**: 只显示当前配置对应的图像数据范围（文件偏移 + 图像大小），而不是整个文件
- 使用搜索功能查找特定的字节模式
